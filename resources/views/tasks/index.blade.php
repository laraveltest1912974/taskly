            ...
            @foreach ($tasks as $task)
                ...
                <div class="flex items-center gap-3.5 px-4 py-3 bg-zinc-950/40 backdrop-blur-md ring-1 ring-white/[0.08] hover:ring-white/[0.14] rounded-xl transition-shadow {{ $isResolved ? 'opacity-60' : '' }}">
                    ...
                    <div class="flex items-center gap-3 pl-2">
                        <a href="{{ route('tasks.edit', $task) }}" class="text-zinc-500 hover:text-brand-300 transition-colors" title="{{ __('Edit') }}">
                        ...
                        <form method="POST" action="{{ route('tasks.destroy', $task) }}"
                            ...
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-zinc-500 hover:text-red-400 transition-colors" title="{{ __('Delete') }}">
                                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75">
