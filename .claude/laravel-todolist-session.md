...
**Produkcija (Render) i instalacija — potvrđeno 2026-09-18:** svi PWA deploy-evi prošli kroz Auto-Deploy (`6d5344b`, `d2c49af`, `892cc05`, ~50 s svaki). Na produkciji je manifest u početku išao kao `application/octet-stream` (alpine nginx ne zna `.webmanifest`); ispravljeno u `deploy/nginx.conf` sa `location = /manifest.webmanifest { default_type application/manifest+json; }` (commit `892cc05`, provereno u kontejneru i na live URL-u). **Korisnik je instalirao PWA u desktop Chrome-u** (opcija *Install and create shortcut*) — radi u sopstvenom prozoru, a **offline stranica (`/offline.html`) se prikazuje kad nema mreže**.

**Nije provereno:** pravi mobilni uređaj (dodir, notch, tastatura) i instalacija na telefonu (Chrome → *Install app*, Safari → *Add to Home Screen*) — probati na `https://taskly-olux.onrender.com`. Guest `/login` se ne prikazuje u iframe-u dok je lokalna sesija prijavljena (preusmerava), pa je guest layout viđen kroz `/privacy`.
